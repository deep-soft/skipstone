...
final class SkipLicenseTests: XCTestCase {
    ...
    func testCrypt() throws {
        ...
        XCTAssertThrowsError(try aes(data: encrypted, encrypt: false, currentDate: DateComponents(calendar: Calendar.current, timeZone: .gmt, year: 2030, month: 1, day: 1).date!), "expected key expiration error") { error in
