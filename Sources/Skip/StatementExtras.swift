...
public struct StatementExtras { // Public because part of other public API
    ...
    func statements(syntax: Syntax, in syntaxTree: SyntaxTree, parent: Statement?) -> (statements: [Statement], replace: Bool) {
        var statements: [Statement] = []
        var replace = false
        for directive in directives {
            switch directive {
            case .insert(let string, let extras):
                statements.append(RawStatement(sourceCode: string, syntax: syntax, extras: extras, in: syntaxTree, parent: parent))
            ...
            case .invalid(let string):
                ...
                statements.append(MessageStatement(message: message, parent: parent))
            default:
                break
            }
        }
